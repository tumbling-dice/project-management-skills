...
このskillは、repoごとに `test_runner` custom agent を用意するためのscaffoldです。共通skill側では特定repoのコマンドを固定しません。検証コマンド、必要な権限、artifact、timeout、失敗時の扱いはrepo内skillまたはrepo内文書へ閉じ込めます。

## 目的

`test_runner` は、実装者とは別の役割として検証コマンドを実行し、結果を証跡として返すagentです。修正、テスト更新、原因調査の深掘り、レビュー判定は担当しません。

このskillは、次を整備するために使います。

- repo固有の `test_runner` custom agent
- `test_runner` が読むrepo内verification skillまたは検証手順文書
- `$verification-workflow` から `test_runner` へ委譲できる入力契約と戻り値
...
- `reviewable-gate-review` へ渡す検証証跡を、実装者の自己申告だけにしたくない。
...
- `$verification-workflow` からの委譲を主な入口として扱い、作成物にはその接続方法を明記する。
...
6. `reviewable-gate-review` が消費できる検証証跡形式を決める。
...
- `.codex/skills/<repo-or-purpose>-verification-workflow/SKILL.md`
- `docs/verification/commands.md`

repo内verification skillと検証文書の両方を作る場合は、重複を避けます。通常は、agentが読む詳細手順をrepo内skillに置き、人間向けの一覧や運用メモをdocsへ置きます。

## `test_runner` agent定義に含める内容

`test_runner` のcustom agent定義には、次を含めます。
...
- `$verification-workflow` のDelegation Packetを受け取り、検証証跡へ統合できる形式で返すこと。

agent定義からは、共通の `subagent-execution` またはrepo内の同等skillを読み込ませます。repo内skillで上書きする場合も、`done` / `blocked` とscope制限は維持します。

既存の `.codex/agents` 形式がない場合でも、ユーザーが作成を求めているなら `.codex/agents/test_runner.toml` を作成します。既存形式がある場合はそれに合わせます。filesystem権限やrepo制約で作成できない場合は、docs-onlyの代替成果物へ黙って落とさず、作成不能理由、作るべきpath、作成予定内容を `blocked` として報告します。

## repo内verification skillに含める内容

repo内verification skillには、次を含めます。
...
- retryしてはいけない条件
- 実行禁止または人間承認が必要な操作
- `$verification-workflow` からのDelegation Packetを受け取る条件
- `reviewable-gate-review` へ渡す検証証跡形式
- 検証結果として返すrepo固有の報告項目

コマンド本文を複数箇所へ複製しません。既存のscriptやCIがsource of truthなら、その参照と用途を書きます。

## Delegation Packetへの追加

Main Agent から `test_runner` へ渡す委譲文は、`subagent-orchestration` の Delegation Packet を使います。`Agent`、`fork_context: false`、`Scope`、`Goal`、`Do not`、`Evidence`、`Deliver`、`Done when` の順を維持します。
...
実行コマンドが空の場合、`test_runner` はrepo内verification skillから候補を提案して `blocked` を返します。推測で広い検証を実行しません。

## 報告形式

`test_runner` の報告形式はrepo内verification skillまたは委譲文で指定します。指定がない場合は、検証専用agentとして次の項目を含めます。
...
検証失敗がある場合は、`Result` または `Verification` に failure classification と次の戻り先を含めます。
...
- 権限昇格が必要な検証
- 実行禁止または人間承認が必要な操作
- `reviewable-gate-review` へ渡す証跡形式
