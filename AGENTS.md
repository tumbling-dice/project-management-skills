...
このrepoは、`~/.codex/skills` で使う共通skillをGit管理するための管理ディレクトリである。

## PJ概要

- 共通skill本体は、このrepo直下の各skillディレクトリで管理する。
- Linux / macOS では、`~/.codex/skills` 側にこのrepo内skillディレクトリへのシンボリックリンクを置く。
- Windows では、`~/.codex/skills` 側にこのrepo内skillディレクトリをコピーする。既存pathがある場合は確認を取り、OKの場合だけ上書きする。
