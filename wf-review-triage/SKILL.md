...
- 指摘を実装する場合。その場合は `wf-implement` へ戻す。
- 新しい実装前調査と計画を作る場合。その場合は `wf-explore` を使う。
- review判定そのものを行う場合。その場合は `wf-review` を使う。
...
5. `wf-implement` へ戻す指摘は、修正scope、変更可能ファイル、必要テスト、再検証、再review入力をまとめる。
6. 計画外の指摘は、勝手に実装scopeへ入れず、戻り先と人間判断の要否を示す。

入力不足で分類できない場合は、指摘を `fix-in-plan` として扱わず、`blocked-input-missing` にする。不足している計画、diff、検証証跡、review sourceを列挙し、必要なら `handoff` へ戻して入力を揃える。
...
全体の `triage_status` は次の基準で選ぶ。

- `ready_for_fix`: 少なくとも1件の `fix-in-plan` があり、その修正scopeを安全に切り出せる。別の指摘が `re-plan`、`human-decision`、`verify-only`、`non-blocking` に分かれていても、Fix Packetに混ぜずBlocked Itemsへ分離できていれば使える。
- `blocked`: 入力不足、指摘の矛盾、承認不足により、実装へ戻すFix Packetを安全に作れない。
- `ready_for_plan`: 主な戻り先が `wf-explore`。
- `ready_for_investigation`: 主な戻り先が `wf-explore`。
- `needs_human_decision`: 主な戻り先が人間判断または `idiot`。
...
- wf-implement / wf-verify / wf-explore / specialist review / handoff / idiot / human decision
```
