...
6. `事実`、`仮説`、`未確認事項` を分ける。
7. 未確認事項が計画確定や修正開始可否を止めるか分類する。
8. 変更方針、変更予定ファイル、テスト方針、検証コマンドを作る。
...
10. 認証、認可、tenant、PII、secret、ログ、外部入力、DB、E2E影響を確認する。
11. 人間レビュー観点と、人間が判断する点を明記する。
12. 修正開始可否を `ready` / `blocked` で出す。
13. 最後のフローとして、同じ作業コンテクストを入力に `$decision-clarification-workflow` の手順と出力形式で人間が答えるべき判断だけを整理する。確認事項がない場合は質問数0として報告する。
